...
class Hand:
    ...
    def rankorderstr(self):
        ...
        if self.type == Hand.TYPE_NP:
            ...
            buf += '%s ' % str8hisym
            for i in range(self.straighthi-1, self.straighthi-5, -1):
                buf += card.ranksym(i)
                buf += ' '
            buf = buf.strip()
            buf += ')'
...
if __name__ == '__main__':
    ...
    foo = Hand()
    count = 0
    for c1 in range(0, 46):
        for c2 in range(c1 + 1, 47):
            for c3 in range(c2 + 1, 48):
                for c4 in range(c3 + 1, 49):
                    for c5 in range(c4 + 1, 50):
                        for c6 in range(c5 + 1, 51):
