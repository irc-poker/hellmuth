...
class Tourney:
    ...
    def incmd(self, cmd):
        ...
        if self.playing:
            # open('/home/hellmuth/t.backup', 'w').write(pickle.dumps(self))
            if run:
                self.run()

    def joingame(self, nick):
        print('Tourney.joingame()')
    ...
    def run(self, newhand = False):
        ...
        while p.cmd.cmd != 'NOOP':
            ...
            # Call any bet
                ...
                if tomake < tocall:
                    self.privout(p.nick,
                                 ...
                                 (tocall,))
                    print('Tourney.run:%s call is too small' %\
                                     (p.nick,))
            ...
            if sendstatus and not p.vacation:
                print('Tourney.run:%s is next to act. (%d to call)' %(self.players[self.next2act].nick, self.curbet - self.players[self.next2act].action))
    ...
    def RAISE(self, p):
        ...
        print('----------------------------------------')
        print('%s raising' % p.nick)
        print('       table.curbet = $%d' % self.curbet)
        print('     table.minraise = $%d' % self.minraise)
        print('     player.lastbet = $%d' % p.lastbet)
        print('    player.bankroll = $%d' % p.bankroll)
        print('      player.inplay = $%d' % p.inplay)
        print('      player.action = $%d' % p.action)
        print('              raise = $%d' % araise)
        print('             tocall = $%d' % tocall)
        print('----------------------------------------')
        ...
        # Check to see if player was fully raised
            ...
            if araise < self.minraise:
                ...
                # All-in raise?
                    ...
                    print('Tourney.RAISE:Setting self.curbet to $%d' % p.action)
                ...
                # Normal raise
                    ...
                    if self.curbet > 0:
                        ...
                        print('Tourney.RAISE:chips = $%d' % chips)
            ...
            for pot in self.pots:
                ...
                for w in winners:
                    ...
                    if leftovers and w in sorted and \
                        ...
                        print('%s gets extra chip' % w.nick)
                        myshare = share + 1
    ...
    def nextactiveseat(self, seat=-1):
        ...
        if oldas >= nplayers:
            oldas = nplayers - 1
        for offset in range(1, nplayers):
    ...
    def newhand(self):
        ...
        if self.nlive() == self.nonvacation():
            ...
            if self.nactive() >= 3:
                ...
                if self.nosb:
                    ...
                    if self.players[self.sb].busted:
                        print('SB eliminated.  Holding button')
                        buttonadv = False

                    # The small blind is last hand's big blind
                    self.sb = self.bb
                    print('Setting SB to %d' % (self.sb,))
                ...
                if self.butflag:
                    print('Resetting Tourney.butflag')
                    self.butflag = False
                    buttonadv = False
            ...
            self.run(True)
            #####print('Tourney.newhand:%s is next to act. (%d to call)' %\
            #####                 (self.players[self.next2act].nick,
            #####                  self.curbet - self.players[self.next2act].action))
            #####
            #####self.pubout('%s is next to act. (%d to call)' %\
    ...
    def postblinds(self):
        ...
        print('Tourney.postblinds()')

        if self.nosb:
            self.nosb = False;
            print('BB eliminated.  Skipping SB')
        else:
    ...
    def deal(self):
        ...
        position = 1
        for n in range(Tourney.nholecards):
            for p in range(len(self.players)):
                ...
                if not player.busted:
                    ...
                    if n == 0:
                        ...
                        position += 1
        for n in range(Tourney.nboardcards):
            acard = self.deck.nextcard()
    ...
    def position(self, player):
        ...
        pos = 1
        nlive = self.nlive()
        for offset in range(nlive):
            seat = (seat + offset) % nlive
            print('Tourney.position: %s in seat %d position %d' %\
